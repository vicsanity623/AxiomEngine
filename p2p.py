...
def sync_with_peer(node_instance, peer_url):
    ...
    try:
        ...
        try:
            peer_list_resp = requests.get(f"{peer_url}/get_peers", timeout=5, headers=headers)
            ...
            logger.debug(f"[P2P Discovery] Could not fetch peer list from {peer_url}")
        ...
        logger.info(f"\033[92m[P2P Sync] Found {len(missing_fact_ids)} new facts. Requesting data...\033[0m")
        ...
        for fact in new_facts_payload:
            if not verify_hash(fact.get('fact_content'), fact.get('fact_id')):
                logger.warning(f"\033[91m[P2P Security] WARNING: Peer {peer_url} sent invalid hash. Dropping.\033[0m")
                continue
        ...
        if facts_added_count > 0:
            logger.info(f"Sync Success. Created {facts_added_count} new local records.")
