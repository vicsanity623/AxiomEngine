"""Construct extraction into a ledger database"""
...
def migrate_fact_content_to_compressed(
    ...
    """Ensure all rows in `facts.fact_content` are stored as compressed BLOBs.

    Convert any legacy plaintext rows (e.g., inserted before compression was enforced
    or via older P2P paths) to compressed format in-place.
    """
    try:
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
...
def update_fact_corroboration(
    ...
    try:
        ...
        if not row:
            return
        original_url, existing_sources_str, _current_score = row
