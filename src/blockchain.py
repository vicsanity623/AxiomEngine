"""COnfigure the blockchain and prepare it for lexical mesh."""
...
def _block_payload(
    ...
    """Execute the canonical payload for block hash (deterministic)."""
    return json.dumps(
        {
...
def create_block(
    ...
    fact_ids should be a list of fact_ids
    ...
    try:
        ...
        block_id = _block_id(
            previous_block_id, new_height, created_at_utc, fact_ids
        )
...
def validate_block(
    ...
    computed_id = _block_id(
        previous_block_id, height, created_at_utc, fact_ids
    )
    if computed_id != block_id:
        return (
            False,
            f"hash mismatch (computed {computed_id[:16]}..., received {block_id[:16]}...)",
        )
...
def append_block(
    ...
    try:
        ...
        if (
            block["previous_block_id"] != prev_id
            or block["height"] != prev_height + 1
        ):
            logger.warning(
        ...
        if not ok:
            logger.warning(
                "[P2P Chain] Append failed: block validation failed (%s).",
                reason,
            )
            return False
        ...
        logger.warning(
            "[P2P Chain] Append failed: block already exists (duplicate block_id)."
        )
...
def replace_chain_with_peer_blocks(
    blocks: list[dict[str, Any]], db_path: str | None = None
) -> bool:
    """Replace our chain (keep genesis) with the peer's blocks.
    ...
    try:
        ...
        for i, blk_wire in enumerate(blocks):
            ...
            if (
                blk["previous_block_id"] != prev_id
                or blk["height"] != prev_height + 1
            ):
                logger.error(
            ...
            ok, reason = validate_block(
                ...
                blk[
                    "created_at_utc"
                ],  # *** FIX: Corrected key from 'blk_created_at_utc' back to 'created_at_utc' ***
                blk["fact_ids"],
                prev_id,
                prev_height + 1,
            )
            if not ok:
                logger.error(
                    f"[Chain Replace] Hash/Content validation failed for block {blk['block_id'][:16]}... Reason: {reason}"
                )
                conn.rollback()
                return False
        ...
        logger.error(
            f"[Chain Replace] Unexpected error during chain replacement: {e}"
        )
        conn.rollback()
        return False
    finally:
